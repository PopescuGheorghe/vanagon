  ...
  describe '.commands' do
    ...
    context 'with extra files to sign' do
      ...
      context 'when success' do
        context 'when macos >= 11' do
          it 'generates signing commands for each file using rsync3 --xattrs' do
            commands = Vanagon::Utilities::ExtraFilesSigner.commands(project._project, mktemp, source_dir)
            expect(commands).to match(
              [
                "rsync -e '/usr/bin/ssh -p 22  -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no' --verbose --recursive --hard-links --links  --no-perms --no-owner --no-group --rsync-path=/usr/local/bin/rsync3 --xattrs $(tempdir)/dir/source_dir/test1/a.rb test@abc:/tmp/xyz",
                "/usr/bin/ssh -p 22  -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no test@abc 'codesign /tmp/xyz/a.rb'",
                "rsync -e '/usr/bin/ssh -p 22  -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no' --verbose --recursive --hard-links --links  --no-perms --no-owner --no-group --rsync-path=/usr/local/bin/rsync3 --xattrs test@abc:/tmp/xyz/a.rb $(tempdir)/dir/source_dir/test1/a.rb",
                "rsync -e '/usr/bin/ssh -p 22  -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no' --verbose --recursive --hard-links --links  --no-perms --no-owner --no-group --rsync-path=/usr/local/bin/rsync3 --xattrs $(tempdir)/dir/source_dir/test2/b.rb test@abc:/tmp/xyz",
                "/usr/bin/ssh -p 22  -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no test@abc 'codesign /tmp/xyz/b.rb'",
                "rsync -e '/usr/bin/ssh -p 22  -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no' --verbose --recursive --hard-links --links  --no-perms --no-owner --no-group --rsync-path=/usr/local/bin/rsync3 --xattrs test@abc:/tmp/xyz/b.rb $(tempdir)/dir/source_dir/test2/b.rb"
              ]
            )
          end
        end
        ...
        context 'when macos < 11' do
          ...
          let(:platform) { Vanagon::Platform::DSL.new('osx-10.15-x86_64') }

          it 'generates signing commands for each file using rsync --extended-attributes' do
            commands = Vanagon::Utilities::ExtraFilesSigner.commands(project._project, mktemp, source_dir)
            expect(commands).to match(
              [
        ...
        context 'when other platform' do
          ...
          it 'generates signing commands for each file using rsync' do
            commands = Vanagon::Utilities::ExtraFilesSigner.commands(project._project, mktemp, source_dir)
            expect(commands).to match(
              [
